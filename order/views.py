...
def addtoshopcart(request):
    ...
    if cart: #on existing cart is noticed 
        ...
        else: #products is not in the cart, add it
            anitem = ShopCart()
...
def shopcart(request):
    ...
    # shipping rules: 0% fees to all orders above 150, 0 fees to orders lower
    if subtotal > 150:
...
def updatequantity(request):
    ...
    messages.success(request,"product Quantity sucessfully updated")
    return redirect(url)
...
def checkout(request):
    ...
    # shipping rules: 8% fees to all orders above 150.0 fees to oreder lower 
    if subtotal > 150:
        shippingfee = 0.08 * subtotal 
    else:
        shippingfee = 0 
...
def placeorder(request):
    api_key = 'sk_test_a531e70ae7fc4a7eaa53bb1004ae731b4e854033'
    url = 'https://api.paystack.co/transaction/initialize'
    callback_url = 'http://18.118.18.73/order/ordercompleted/'
